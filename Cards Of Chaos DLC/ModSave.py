...
HMAC_SECRET = b"CardsOfChaos"            # Secret for tamper check
SAVE_FILE_PATH = os.path.join("Saves", "Modsave.bin")
...
def encode_save_file(save_data=None):
    ...
    if not save_data:
        ...
        save_data = {
                'Cards': [(0, False), (1, False), (2, False), (3, False)], # Cards collected
                'Deck': [],  # Cards in the deck, max 4
                'World': 0, # Current world {0: Enchanter, 1: Monarch, 2: Madman, 3:Clockwork}
                'Level': 0, # Current level in the world
                'lives': 3, # Lives for the run
                'Cracked': False, # If the mirror has been cracked
            }
...
def decode_save_file() -> dict[str, any]:
    """
...
def resetRun():
    """
    Initialize or reset the mod save to a clean run-start state and persist to disk.
    Called when starting a new run (ModSave.resetRun()).
    """
    try:
        save = decode_save_file() or {}
    except Exception:
        save = {}

    # Minimal safe defaults for a fresh run
    save.setdefault('GameVersion', VERSION if 'VERSION' in globals() else "0.0.0")
    save['modded'] = True
    save['World'] = 1
    save['Level'] = 0
    save['lives'] = 3
    save['gold'] = 0
    save['NumWins'] = 0
    # Keep Cards key if present, otherwise create empty unlocked list structure
    if 'Cards' not in save or not isinstance(save['Cards'], list):
        # store as list of (id, guilded_flag)
        save['Cards'] = []
    # Reset deck/hand to empty so UI BuildDeck will populate/assign defaults
    save['Deck'] = []
    save['Hand'] = []
    save['RunEnded'] = False
    ...
    try:
        encode_save_file(save)
    except Exception:
        # best-effort persistence; swallow errors so caller can continue
        pass
